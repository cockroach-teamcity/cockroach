...
type drpcMockReceiver struct {
	*mockReceiver
}

func (s *drpcMockReceiver) PushUpdates(stream ctpb.DRPCSideTransport_PushUpdatesStream) error {
	if s.called.CompareAndSwap(false, true) {
		close(s.calledCh)
	}
	<-s.stop
...
func (ln *dropDRPCHeaderListener) Accept() (net.Conn, error) {
	...
	ln.mu.Lock()
	ln.conns = append(ln.conns, conn)
	ln.mu.Unlock()
	return conn, nil
}

func (ln *dropDRPCHeaderListener) CloseConns() {
	ln.mu.Lock()
	defer ln.mu.Unlock()
...
func newMockSideTransportDRPCServerWithOpts(
	...
	drpcL := &dropDRPCHeaderListener{Listener: lis}

	mux := drpcmux.New()
	srv := drpcserver.New(mux)

	drpcImpl := &drpcMockReceiver{mockReceiver: receiver}
	if err := ctpb.DRPCRegisterSideTransport(mux, drpcImpl); err != nil {
		return nil, err
	}

	server := &sideTransportDRPCServer{
		lis:      lis,
		srv:      srv,
		receiver: receiver,
	}
	...
	if err := stopper.RunAsyncTask(ctx, "drpc-server", func(ctx context.Context) {
		if err := srv.Serve(ctx, drpcL); err != nil {
...
func (m *mockDialer) DRPCDial(
	...
	c := drpcconn.New(rawConn)
	m.mu.Lock()
	m.mu.drpcConns = append(m.mu.drpcConns, c)
	m.mu.Unlock()
	return c, nil
}
...
// Test a Sender and Receiver talking gRPC/DRPC to each other.
	...
	testutils.RunTrueAndFalse(t, "useDRPC", func(t *testing.T, useDRPC bool) {
		...
		for i := 0; i < numNodes; i++ {
			...
			if useDRPC {
				...
				mux := drpcmux.New()
				srv := drpcserver.New(mux)
				require.NoError(t, ctpb.DRPCRegisterSideTransport(mux, receivers[i].AsDRPCServer()))
				receiverStop.OnQuiesce(func() {
					_ = lis.Close()
					drpcL.CloseConns()
				})
				require.NoError(t, receiverStop.RunAsyncTask(ctx, "drpc-server", func(ctx context.Context) {
					if err := srv.Serve(ctx, drpcL); err != nil {
						log.Dev.Infof(ctx, "drpc server stopped: %v", err)
					}
				}))
				dialer.addOrUpdateNode(nid.Get(), lis.Addr().String())
			} else {
